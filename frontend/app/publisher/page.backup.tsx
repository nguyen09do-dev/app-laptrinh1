...
export default function MultiPlatformPublisherPage() {
  ...
  const fetchLibraryContents = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.log('📚 Loaded library contents:', contentsWithText.length, 'items');
  ...
  const generateDerivatives = async () => {
    ...
    try {
      ...
      const bodyKey = 'content_id';
      const bodyValue = selectedContent.content_id;
      ...
      if (data.success && data.derivatives) {
        ...
        setSelectedContent({ ...selectedContent!, derivatives: data.derivatives });
  ...
  const handlePlatformPublish = async (platform: PlatformConfig) => {
    if (!selectedContent || !selectedContent.derivatives) {
  ...
  // Wizard navigation
  const goToNextStep = () => {
    if (currentStep === 'select' && selectedContent) {
      if (selectedContent.derivatives) {
        setCurrentStep('publish');
      } else {
        setCurrentStep('generate');
      }
    } else if (currentStep === 'generate' && selectedContent?.derivatives) {
      setCurrentStep('publish');
    }
  };
  ...
  const handleContentSelect = (item: LibraryContent) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-midnight-900 via-midnight-800 to-midnight-900">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        {/* Wizard - Only ONE step visible at a time */}
        <AnimatePresence mode="wait">
            ...
            >
              ...
              {libraryContents.length === 0 && (
                <div className="text-center py-12 mb-6">
                  ...
                  <p className="text-midnight-400 mb-2">No content available in library</p>
                  <p className="text-sm text-midnight-500">
                    Create content in Content Studio first, then come back here to publish
                  </p>
                </div>
              )}
          ...
          {currentStep === 'generate' && selectedContent && (
            <motion.div
              key="generate"
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              <StepHeader
                ...
                nextDisabled={!selectedContent.derivatives}
              />

              {selectedContent.derivatives ? (
          ...
          {/* Step 3: Publish */}
          {currentStep === 'publish' && selectedContent?.derivatives && (
            <motion.div
              key="publish"
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
            ...
            >
              ...
              <PlatformTabs
                ...
                derivatives={selectedContent.derivatives}
